// ─────────────────────────────────────────────────────────────────────────────
// lib/services/ai/pdfExtractor.ts — PDF text extraction
//
...
//                            Uses the same GOOGLE_CLIENT_EMAIL / GOOGLE_PRIVATE_KEY
//                            service account as the Drive service.
//
//   2. pdf-parse + Claude  — fallback (or primary when Google Doc AI is not
//                            configured). Extracts raw text with pdf-parse,
//                            then asks Claude to return structured JSON.
//
...
// ── Strategy 1: pdf-parse + Claude ───────────────────────────────────────────

async function extractWithClaude(rawText: string): Promise<ExtractedInvoiceFields> {
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `You are an invoice data extractor. From the invoice text below, extract the fields and return ONLY valid JSON — no markdown, no explanation.

Invoice text:
${rawText.slice(0, 6000)}

Return exactly this JSON structure (use null for any field you cannot find with confidence):
{
...
Rules:
- All monetary amounts must be plain numbers (no currency symbols or commas)
- invoiceDate must be YYYY-MM-DD format
- taxRate is a decimal (0.1 for 10%, 0.08 for 8%)`,
      },
    ],
  });
  ...
  try {
    ...
    // Claude returned unparseable JSON — return empty fields with raw text
  }
...
export async function extractWithPdfParseAndClaude(
  pdfBytes: Uint8Array
  ...
  if (!rawText.trim()) {
    console.warn("[pdfExtractor] pdf-parse returned empty text (scanned image?)");
    return emptyExtracted();
  }

  return extractWithClaude(rawText);
}
...
async function getGoogleAccessToken(): Promise<string> {
  ...
  if (!clientEmail || !privateKey) {
    throw new Error(
      "GOOGLE_CLIENT_EMAIL and GOOGLE_PRIVATE_KEY are required for Google Document AI"
    );
  }

  // Build service-account JWT and exchange it for an OAuth2 access token
  ...
  if (!tokenRes.ok) {
    throw new Error(`Google OAuth token request failed: ${tokenRes.status}`);
  }
...
// ── Main entry point ──────────────────────────────────────────────────────────
//
// Tries Google Document AI first (if configured), falls back to pdf-parse + Claude.
...
export async function extractFromPdf(pdfBytes: Uint8Array): Promise<ExtractedInvoiceFields> {
  ...
  return extractWithPdfParseAndClaude(pdfBytes);
}
