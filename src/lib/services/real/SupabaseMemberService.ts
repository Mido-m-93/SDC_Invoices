...
function toRow(m: Member): Record<string, unknown> {
  return { id: m.id, display_name: m.displayName, email: m.email, phone: m.phone, role: m.role, department: m.department, employee_code: m.employeeCode, join_date: m.joinDate, status: m.status, avatar_url: m.avatarUrl, notes: m.notes, created_at: m.createdAt, updated_at: m.updatedAt, contract_start: m.contractStart || null, contract_end: m.contractEnd || null, contracted_amount: m.contractedAmount ?? null, contract_scope: m.contractScope ?? null, contract_sync_attempted_at: m.contractSyncAttemptedAt ?? null };
}

function fromRow(r: Record<string, unknown>): Member {
  return { id: r.id as string, displayName: r.display_name as string, email: r.email as string, phone: r.phone as string, role: r.role as Member["role"], department: r.department as string, employeeCode: r.employee_code as string, joinDate: r.join_date as string, status: r.status as MemberStatus, avatarUrl: r.avatar_url as string, notes: r.notes as string, createdAt: r.created_at as string, updatedAt: r.updated_at as string, contractStart: (r.contract_start as string) ?? null, contractEnd: (r.contract_end as string) ?? null, contractedAmount: (r.contracted_amount as number) ?? null, contractScope: (r.contract_scope as string) ?? null, contractSyncAttemptedAt: (r.contract_sync_attempted_at as string) ?? null };
}
