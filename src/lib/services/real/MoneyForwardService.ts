...
export interface MFSendPayload {
  ...
  currency?: "JPY" | "USD";
  memo?: string;
  sourceUrl?: string;    // link to the original submitted PDF, for traceability
}
...
export class MoneyForwardService {
  ...
  private async createBilling(
    ...
    // Link back to the original submitted document so AP staff can click
    // straight through to it, instead of just seeing its filename.
    const memo = [payload.memo, payload.sourceUrl ? `Source: ${payload.sourceUrl}` : null]
      .filter(Boolean)
      .join(" / ")
      .slice(0, 450); // MF's memo field caps at 450 chars
