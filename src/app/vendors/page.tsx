...
export default function VendorsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(t("vendors_error_load_failed"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(t("vendors_error_save_failed"));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm(t("vendors_confirm_delete"))) return;
  ...
  async function handleImport() {
    ...
    try {
      ...
      setImportMsg(t("vendors_import_complete", { added: data.added ?? 0, skipped: data.skipped ?? 0 }));
      load();
    } catch (e) {
      setError(String(e));
    } finally {
    ...
    <AppShell>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-stone-200 px-6 py-12 text-center">
          <p className="text-stone-400 text-sm">{t("vendors_empty_state")}</p>
          <Button variant="primary" className="mt-4" onClick={openNew}>{t("vendors_empty_cta")}</Button>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 text-xs text-stone-500 uppercase tracking-wide">
              <tr>
                ...
                <th className="px-4 py-3 text-left">{t("vendors_col_tax_reg_no")}</th>
                ...
                <tr key={v.id} className="hover:bg-stone-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${v.status === "active" ? "bg-green-100 text-green-700" : "bg-stone-100 text-stone-500"}`}>
                      {v.status === "active" ? t("vendors_status_active") : t("vendors_status_inactive")}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/30 backdrop-blur-[1px]">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-y-auto max-h-[90vh]">
            <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
              <h2 className="text-base font-semibold">{editing ? t("vendors_modal_title_edit") : t("vendors_modal_title_add")}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <Field label={t("vendors_field_tax_reg_no")}>
                <input className={input} value={form.taxRegistrationNumber} onChange={(e) => set("taxRegistrationNumber", e.target.value)} placeholder={t("vendors_field_tax_reg_no_placeholder")} />
              </Field>
            ...
            <div className="px-6 py-4 border-t border-stone-100 flex justify-end gap-3">
              ...
              <Button variant="primary" loading={saving} onClick={handleSave}>{t("vendors_save_vendor_button")}</Button>
            </div>
          </div>
        </div>
      )}
