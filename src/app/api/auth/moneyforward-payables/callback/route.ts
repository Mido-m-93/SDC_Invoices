// GET /api/auth/moneyforward-payables/callback - exchange authorization code for tokens
import { NextRequest, NextResponse } from "next/server";
import { exchangeMFPayablesCode } from "@/lib/services/real/MoneyForwardPayablesService";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const tokens = await exchangeMFPayablesCode(code);
    ...
    try {
      ...
      const { error } = await db.storage.from("mf-config").upload("payables_tokens.json", payload, {
        upsert: true,
        contentType: "application/json",
      });
      if (error) throw new Error(JSON.stringify(error));
      savedToDb = true;
      console.log("[MF Payables callback] Tokens saved to Storage");
    } catch (dbErr) {
      dbError = dbErr instanceof Error ? dbErr.message : JSON.stringify(dbErr);
      console.error("[MF Payables callback] Failed to save tokens:", dbError);
    }

    return new NextResponse(
      html(`
        <h2>Money Forward Payables connected! &check;</h2>
        <p>${savedToDb
        ...
        ${!savedToDb ? `<table>
          <tr>
            <td><code>MF_PAYABLES_ACCESS_TOKEN</code></td>
            <td><textarea rows="2" style="width:600px">${tokens.accessToken}</textarea></td>
          </tr>
          <tr>
            <td><code>MF_PAYABLES_REFRESH_TOKEN</code></td>
...
function html(body: string): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Money Forward Payables Auth</title>
