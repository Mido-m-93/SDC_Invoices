...
export async function POST(req: NextRequest) {
  try {
    ...
    const client: Client = { id: body.id || generateId("cli"), name: body.name ?? "", legalName: body.legalName ?? "", industry: body.industry ?? "", contactName: body.contactName ?? "", contactEmail: body.contactEmail ?? "", contactPhone: body.contactPhone ?? "", address: body.address ?? "", country: body.country ?? "JP", taxRegistrationNumber: body.taxRegistrationNumber ?? "", status: body.status ?? "prospect", notes: body.notes ?? "", createdAt: body.createdAt ?? now, updatedAt: now };
