...
export default function ExceptionsPage() {
  ...
  const loadMonths = useCallback(async () => {
    try {
      ...
      setError(t("exceptions_error_load_months"));
    }
  }, []);
  ...
  const loadExceptions = useCallback(async (month: string) => {
    ...
    try {
      ...
      setError(t("exceptions_error_load_invoices"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <SummaryCard label={t("exceptions_total_exceptions")} value={items.length} color="stone" />
        <SummaryCard label={t("exceptions_blocked")} value={blocked} color="red" />
        <SummaryCard label={t("exceptions_needs_review")} value={needsReview} color="amber" />
      </div>
      ...
      <div className="flex gap-2 mb-4">
        {(["ALL", "BLOCKED", "NEEDS_REVIEW"] as FilterLevel[]).map((f) => (
          ...
          >
            {f === "ALL" ? t("exceptions_all") : f === "BLOCKED" ? t("exceptions_blocked") : t("exceptions_needs_review")}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-stone-200 px-6 py-12 text-center">
          <p className="text-stone-400 text-sm">
            {items.length === 0 ? t("exceptions_empty_no_exceptions") : t("exceptions_empty_no_match")}
          </p>
        </div>
      ) : (
                  ...
                  return (
                    <tr key={item.submission.id} className="hover:bg-stone-50">
                      ...
                      <td className="px-4 py-3 text-xs text-stone-500 max-w-xs">
                        {v?.issues?.length ? v.issues.map((i) => translateIssue(i, language)).join(", ") : "—"}
                      </td>
